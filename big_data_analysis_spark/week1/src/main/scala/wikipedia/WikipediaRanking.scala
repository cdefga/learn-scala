...
object WikipediaRanking extends WikipediaRankingInterface {
  ...
  val conf: SparkConf = new SparkConf().setMaster("local").setAppName("Week 1")
  ...
  def occurrencesOfLang(lang: String, rdd: RDD[WikipediaArticle]): Int = {
    rdd.filter(_.mentionsLanguage(lang)).aggregate(0)((accu, _) => accu + 1, _ + _)
  }
  ...
  def makeIndex(langs: List[String], rdd: RDD[WikipediaArticle]): RDD[(String, Iterable[WikipediaArticle])] = {
    rdd.flatMap(x => langs.filter(x.mentionsLanguage).map((_, x))).groupByKey
  }
  ...
  def rankLangsReduceByKey(langs: List[String], rdd: RDD[WikipediaArticle]): List[(String, Int)] = {
    rdd.flatMap(x => langs.filter(x.mentionsLanguage).map((_, 1))).reduceByKey(_ + _).sortBy(-_._2).collect.toList
  }
