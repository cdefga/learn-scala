...
object TimeUsage extends TimeUsageInterface {
  ...
  def timeUsageByLifePeriod(): Unit = {
    ...
    // columns.take(5).foreach(println)
    // initDf.take(5).foreach(println)
    val (primaryNeedsColumns, workColumns, otherColumns) = classifiedColumns(columns)
    // primaryNeedsColumns.take(5).foreach(println)
  ...
  def classifiedColumns(columnNames: List[String]): (List[Column], List[Column], List[Column]) = {
    ...
    val others  = List("t02", "t04", "t06", "t07", "t08", "t09", "t10", "t12", "t13", "t14", "t15", "t16", "t18" )

    val grouped = columnNames.groupBy(col => {
        if (primary.exists(col.startsWith)) "primary"
        else if (working.exists(col.startsWith)) "work"
        else if (others.exists(col.startsWith)) "other"
        else "unused"
  ...
  def timeUsageGroupedSqlQuery(viewName: String): String =
    s"""SELECT working, sex, age,
       ...
       | ORDER BY working, sex, age""".stripMargin.replaceAll("\n", " ")
